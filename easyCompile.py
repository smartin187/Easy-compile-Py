...
def easyCompile(
    ...
    * title : the title of windows. Defaut is "Easy compile Py".
    ...
    class Trad:
        ...
        t107 = {
            "fr":"Une erreur est survenue lors de la création de l'instalateur...",
            "en":"An error occurred during the creation of the installer..."
        }
        ...
        t109 = {
            ...
            "en":"Missing dependence"
        }

        t110 = {
            "fr":"La librairie pywin32 est néssessaire pour crée l'instalateur.\nPour l'installer, écriver dans un terminale :\npip install pywin32\n",
            "en":"The pywin32 library is necessary to create the installer.\nFor install if, set on a terminal:\npip install pywin32\n"
        }

        t111 = {
            "fr":"Voulez vous l'installer automatiquement ?",
            "en":"Do you want to install automatically it ?"
        }
        ...
        t113 = {
            "fr":"Impossible d'installer pywin32. Esseyer l'instalation manuelle...\nVérifier que Python et pip sont bien installer...",
...
try:
    ...
    def configure_frame_windows(frame: tk.Frame) -> None:
        ...
        list_compiling = ttk.Combobox(frame_type_compiling, values=(text_type_of_compile["exe"], text_type_of_compile["exeinstaller"]), state="readonly", width=30)
        ...
        button_compile = tk.Button(frame_compile, activebackground=BG, activeforeground=FG, text=Trad.t005[language], command=compile_windows, font=("Arial", 25))
        button_compile.pack()
    ...
    def configure_frame_Linux(frame: tk.Frame) -> None:
        ...
        def compile_Linux():
            ...
            if os_name == "linux":
                        ...
                        if not pathlib.Path("appimagetool.appimage").is_file():
                            ...
                            if install_appimagetool:
                                ...
                                if automatic_install:
                                    ...
                                    button_step_0 = tk.Button(step[0], activebackground=BG, activeforeground=FG, text=Trad.t054[language], command=lambda: webbrowser.open("https://github.com/AppImage/appimagetool/releases")).pack()

                                    button_close = tk.Button(window_manual_install_appimage, activebackground=BG, activeforeground=FG, text=Trad.t057[language], command=window_manual_install_appimage.destroy).pack(pady=10)
                ...
                if compile_ok :
                    ...
                    button_save = tk.Button(frame_button_compile, activebackground=BG, activeforeground=FG, text=Trad.t027[language], command=lambda: save_compile(".deb" if deb_ok else ".AppImage" if appimage_ok else "")).grid(column=0, row=0)
                    ...
                    if result.returncode!=0:
                        ...
                        if install_pyinstaller:
                            ...
                            if automatic_install_pyinstaller:
                                ...
                                button_close_guide = tk.Button(window_install_pyinstaller, activebackground=BG, activeforeground=FG, text=Trad.t098[language], command=window_install_pyinstaller.destroy).pack(pady=10)
        ...
        def set_frame_config() -> None:
            ...
            if list_compiling.get() == text_type_of_compile["deb"] and (frame_config_type=="bin" or frame_config_type=="appimage"):
                ...
                button_icon = tk.Button(appimage_column_0, activebackground=BG, activeforeground=FG, text=Trad.t063[language], command=set_icon).grid(column=1, row=1)
        ...
        button_compile = tk.Button(frame_compile, activebackground=BG, activeforeground=FG, text=Trad.t005[language], command=compile_Linux, font=("Arial", 25))
        button_compile.pack()
    ...
    text_type_of_compile = {
        ...
        "exeinstaller":"Windows Executable Installer | *.exe",
                                            # Linux :
    ...
    try:
        ...
        def automatic_installe() -> None:
            ...
            def error_install(détail: str) -> None:
                ...
                #button_python = tk.Button(frame_button_error, activebackground=BG, activeforeground=FG, text=Trad.t018[language], command=open_python_on_webbrowser).grid(column=0, row=0)
                button_close = tk.Button(frame_button_error, activebackground=BG, activeforeground=FG, text=Trad.t019[language], command=window_easy_compile.destroy).grid(column=1, row=0)
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    button_ok = tk.Button(window_pyinstaller_install, activebackground=BG, activeforeground=FG, text=Trad.t015[language], command=window_easy_compile.destroy).pack()
        ...
        def manual_install() -> None:
            ...
            #button_install_python = tk.Button(frame_install_1, activebackground=BG, activeforeground=FG, text=Trad.t018[language], command=open_python_on_webbrowser).pack()
            ...
            button_close = tk.Button(window_manual_install, activebackground=BG, activeforeground=FG, text=Trad.t019[language], command=window_manual_install.destroy).pack()
        ...
        button_automatic_installe = tk.Button(frame_button, activebackground=BG, activeforeground=FG, text=Trad.t013[language], command=automatic_installe).grid(column=0, row=0)

        button_manual_intall = tk.Button(frame_button, activebackground=BG, activeforeground=FG, text=Trad.t020[language], command=manual_install).grid(column=1, row=0)

        button_close = tk.Button(frame_button, activebackground=BG, activeforeground=FG, text=Trad.t019[language], command=window_easy_compile.destroy).grid(column=2, row=0)
